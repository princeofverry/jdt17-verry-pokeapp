  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8 flex-1 flex flex-col">
      ...
      <div className="mb-8 text-center sm:text-left select-none">
        <h1 className="font-heading text-3xl font-black tracking-tight text-foreground sm:text-4xl flex items-center justify-center sm:justify-start gap-2.5 uppercase">
      ...
      {collection.length > 0 ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 flex-1 items-start">
          {collection.map((pokemon) => {
            ...
            return (
              <Card key={`${pokemon.id}-${pokemon.nickname}`} className={`group overflow-hidden border-4 bg-card text-foreground hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[7px_7px_0px_#000] transition-all duration-200 ${borderColorClass}`}>
                ...
                <div className="relative flex aspect-square w-full items-center justify-center bg-muted p-4 border-b-3 border-black">
                  {pokemon.image ? (
                    ...
                    <div className="h-[100px] w-[100px] bg-card border-2 border-black rounded-lg flex items-center justify-center text-foreground font-black">
                      N/A
                    </div>
                  )}
                ...
                {/* Nickname & Species */}
                <div className="p-3 w-full text-center bg-card">
                  <h3 className="font-heading text-sm font-black text-foreground truncate">
                    {pokemon.nickname}
                  </h3>
                  <p className="text-[9px] text-muted-foreground font-extrabold tracking-wider uppercase mt-0.5 truncate">
                  ...
                  {/* Date details */}
                  <p className="mt-2 text-[9px] font-bold text-muted-foreground select-none">
                  ...
                  <div className="mt-4 flex gap-1.5 w-full justify-center items-center">
                    <Link
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-1.5 h-8 text-[10px] font-black border-2 border-black rounded-lg text-black bg-accent shadow-[2px_2px_0px_#000] hover:-translate-x-[1px] hover:-translate-y-[1px] hover:shadow-[3px_3px_0px_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all cursor-pointer select-none"
                    >
        ...
        <div className="py-12 flex flex-col items-center justify-center text-center select-none bg-card border-4 border-black shadow-[6px_6px_0px_#000] rounded-xl p-8 max-w-md mx-auto my-12">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground border-2 border-black shadow-[2px_2px_0px_#000] mb-6">
            <Trophy className="h-6 w-6" />
          </div>
          <h3 className="font-heading text-xl font-black text-foreground">Your collection is empty</h3>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3 w-full justify-center">
            <Link href="/" className="flex-1">
              <Button variant="outline" className="w-full font-black h-10 cursor-pointer shadow-[3px_3px_0px_#000] hover:shadow-[4px_4px_0px_#000]">
                <Compass className="h-4 w-4 mr-1 text-foreground" />
                Go to Pokedex
              </Button>
            </Link>
            <Link href="/gacha" className="flex-1">
              <Button className="w-full font-black h-10 bg-primary text-primary-foreground hover:bg-primary border-2 border-black shadow-[3px_3px_0px_#000] hover:shadow-[4px_4px_0px_#000]">
                <Trophy className="h-4 w-4 mr-1 text-primary-foreground" />
                Gacha Roll
              </Button>
            </Link>
          </div>
      ...
      <Dialog open={showConfirmRelease} onOpenChange={setShowConfirmRelease}>
        <DialogContent className="max-w-sm w-[95%] sm:w-full rounded-xl border-4 border-black shadow-[8px_8px_0px_#000] bg-card p-6 text-foreground">
          <DialogHeader>
            <DialogTitle className="font-heading font-black text-lg text-foreground flex items-center gap-1.5 select-none uppercase">
              <ShieldAlert className="h-5 w-5 text-destructive" />
              Release Pokemon?
            </DialogTitle>
            <DialogDescription className="text-muted-foreground font-bold text-sm">
              Are you sure you want to release <span className="font-black text-foreground">{selectedPokemon?.nickname}</span> the <span className="font-black text-foreground">{selectedPokemon ? capitalize(selectedPokemon.name) : ""}</span> back into the wild? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="flex-row gap-3 mt-4">
            <Button
              ...
              className="flex-1 font-black rounded-lg border-2 border-black shadow-[2px_2px_0px_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none hover:-translate-y-0.5 transition-all bg-card text-foreground select-none"
            >
              Cancel
            </Button>
            <Button
              ...
              className="flex-1 font-black rounded-lg border-2 border-black shadow-[2px_2px_0px_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none hover:-translate-y-0.5 transition-all bg-destructive text-white select-none"
            >
              Release
            </Button>
          </DialogFooter>
